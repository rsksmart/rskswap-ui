<template>
  <div class="row">
    <div class="offset-md-2 col-md-8 col-12">
      <p
        v-if="showSendToContractWarning"
  ...
  <div class="row">
    <div class="offset-md-2 col-md-8 col-12">
      <p v-if="showAddressWarning" class="alert alert-warning" role="alert">
        Sending founds directly to exchange may result in loses if they don't
        support transactions sent by smart contracts
      </p>
    </div>
  </div>
  ...
  <Modal v-if="showModal" @close="showModal = false">
    ...
    <template #body>
      ...
      <p>
        Once it has enough confirmation you will need to
        <b>claim it on the other network</b>
      </p>
...
export default {
  ...
  computed: {
    confirmations() {
      ...
      const confirmations = config.isRsk
      ...
      if (amount.isLessThan(limit.mediumAmount)) {
        dataToReturn = {
          ...
          time: moment
            .duration(
              confirmations.smallAmount * config.secondsPerBlock,
              "seconds"
            )
        ...
        dataToReturn = {
          ...
          time: moment
            .duration(
              confirmations.mediumAmountTime * config.secondsPerBlock,
              "seconds"
            )
            .add(2, "minutes") // voting time
            .humanize(),
        ...
        dataToReturn = {
          ...
          time: moment
            .duration(
              confirmations.largeAmountTime * config.secondsPerBlock,
              "seconds"
            )
            .add(2, "minutes") // voting time
            .humanize(),
    ...
    accountConnected() {
      return `${this.sharedState.chainId || ""} ${
        this.sharedState.accountAddress || ""
      } ${this.sharedState.sideConfig?.networkId || ""}`.trim();
    },
    fee() {
    ...
    destinationNetwork() {
      return (
        this.sharedState.currentConfig?.crossToNetwork ||
    ...
    waitSeconds() {
      if (
        !this.sharedState.currentConfig ||
        !this.selectedTokenMediumAmount ||
        !this.amount
      ) {
        return 0;
      }
  ...
  methods: {
    async handleChangeNetwork(network) {
      try {
        ...
        this.destinationNetworks = networksConf?.networks.map(
          (record) => record.crossToNetwork
        );
      } catch (error) {
        if (error.code === 4902) {
    ...
    handleChangeDestinationNetwork(network) {
      ...
      const rskConfig = originNetwork.isRsk
        ? originNetwork
        : originNetwork.crossToNetwork;
      const sideConfig = rskConfig.crossToNetwork;
      if (
        this.originNetworkSelected.crossToNetwork.networkId !==
        network.networkId
      ) {
    ...
    async handleSwitchNetwork() {
      ...
      try {
        ...
        const chainId = numToHex(
          this.sharedState.currentConfig.crossToNetwork.networkId
        );
        await window.ethereum.request({
          method: "wallet_switchEthereumChain",
          params: [{ chainId }],
    ...
    async refreshBalanceAndAllowance() {
      ...
      if (token.methodType === METHOD_TYPES.DEPOSITOR) {
        ...
        this.selectedTokenBalance = new BigNumber(ethBalance).shiftedBy(
          -decimals
        );
        if (
          new BigNumber(this.amount).isGreaterThan(this.selectedTokenBalance)
        ) {
    ...
    async selectToken(token, event) {
      ...
      //   .call()
      const limits =
        this.sharedState.networkSettings.typesLimits[this.selectedToken.typeId];
      this.selectedTokenMaxLimit = new BigNumber(limits.max).shiftedBy(
        -token.decimals
      );
      this.selectedTokenMinLimit = new BigNumber(limits.min).shiftedBy(
        -token.decimals
      );
      this.selectedTokenMediumAmount = new BigNumber(
        limits.mediumAmount
      ).shiftedBy(-token.decimals);
      this.selectedTokenLargeAmount = new BigNumber(
        limits.largeAmount
      ).shiftedBy(-token.decimals);
    ...
    async setMaxAmount(event) {
      ...
      this.maxAmount = maxAmount.toFixed(
