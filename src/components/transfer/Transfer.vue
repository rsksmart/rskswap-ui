<template>
  <div class="transfer">
    ...
    <div class="transfer-body">
      <div class="row">
        <div class="col-md-12">
          <label class="tokenAddress-label" for="networks"
            >Origin Network</label
          >
          ...
          <div class="form-group amount">
            ...
            <div v-if="isOrigin">
              <RangeInput
                ...
                step="1"
                :disabled="
                  !currentNetwork || !isOrigin || !selectedToken?.token
                "
              />
            </div>
...
export default {
  ...
  computed: {
    ...
    selectedNetwork() {
      return !this.disabled && this.isOrigin
        ? this.sharedState.currentConfig.networkId
        : "";
    },
    maxAmountBigNumber() {
  ...
  watch: {
    percentage(newPercentage) {
      ...
      const amount = this.maxAmountBigNumber
    ...
    networks: {
      ...
      handler(newValue) {
        if (newValue.length > 0) {
          this.currentNetwork = newValue.find(
            (network) => network.networkId === this.chainId
          );
