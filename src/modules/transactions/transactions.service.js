...
export class TransactionService {
  static validateTimestamp(timestamp) {
    if (
      timestamp &&
      timestamp.toString().length < Date.now().toString().length
    ) {
  ...
  async synchronizeTransactions(
    ...
    const transactions = TXN_Storage.getAllTxns4Address(
      accountAddress,
      hostConfig.localStorageName
    );
    const migratedTransactions = transactions.map((transaction) => {
      ...
      transaction.type =
        transaction.type === "Cross Token"
          ? "Cross"
          : transaction.type ?? "Cross";
      // Check if empty amount
      transaction.receiveAmount =
        transaction.receiveAmount ?? transaction.amount;
      // Check if empty sender
      transaction.senderAddress = transaction.senderAddress ?? transaction.from;
      // Check if empty receiver
      transaction.receiverAddress =
        transaction.receiverAddress ?? transaction.senderAddress;
      // Check if its in seconds
      transaction.timestamp = TransactionService.validateTimestamp(
        transaction.timestamp
      );
      transaction.destinationChainId =
        hostConfig.networkId === transaction.networkId
          ? sideConfig.networkId
          : hostConfig.networkId;
    ...
    const storageKey = TXN_Storage.createStorageKey(
      accountAddress,
      hostConfig.localStorageName
    );
  ...
  async saveTransaction(transaction) {
    ...
    try {
      const transactionSaved = await dbInstance.transactions.put(
        fixedTransactionStructure
      );
      if (
        !transactionSaved ||
        transactionSaved !== transaction.transactionHash
      ) {
        return {};
      }
      return transaction;
