...
export const getSwapData = async (web3: any, account: string, tokenName: string) => {
    const token = findTokenByName(tokenName);
    if (!token) throw Error("Token not found") // TODO: Display a message to the user.
    let balance;

    if (token.type === "NATIVE") {
        balance = new BigNumber(await web3.eth.getBalance(account)).shiftedBy(-token.decimals);
    }
    else{
        const tokenInst = new web3.eth.Contract(ERC20_ABI, token.address);
        balance = new BigNumber(await tokenInst.methods.balanceOf(account).call()).shiftedBy(-token.decimals);
    }
...
export const getTokenAllowance = async (web3: any, token: Token, spenderAddress: string) => {
    if (token.type === "NATIVE") { 
